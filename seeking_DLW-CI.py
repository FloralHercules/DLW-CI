...
def _build_paper_main_table(ablation_rows):
    ...
    # 生成 LaTeX
    lines = []
    lines.append("% Auto-generated by DLW-CI paper pipeline")
...
def _build_harmful_table(harmful_rows):
    ...
    mode_labels = {"cognitive_fusion": "Cog. Fusion", "gated_cognitive_fusion": "Gated Cog. Fusion", "full": "Full DLW-CI"}
    lines = []
    lines.append("\\begin{table}[t]")
    lines.append("\\centering")
...
def _generate_experiment_summary(ablation_rows, harmful_rows, stability_note):
    ...
    lines = []
    lines.append("=" * 60)
    lines.append("DLW-CI Paper Experiment Summary")
    lines.append("=" * 60)
    lines.append("")
    # 找最优
    modes_order = list(ABLATION_CONFIGS.keys())
...
def summarize_paper_results():
    """论文级结果汇总主函数."""
    print("\n" + "=" * 60)
    print("DLW-CI: Generating Paper-Ready Results")
    print("=" * 60)
    ablation_rows = _read_csv_safe("ablation_summary.csv")
    harmful_rows = _read_csv_safe("harmful_summary.csv")
...
def dif22_vectorized(X, Y, ln, lo_ids, lo_sources, p_x, p_y, p_q, a, V, D, lamda, D_KL,
    ...
    for i, order_i in enumerate(lo_ids):
        ...
        if fusion_weights is not None:
            # DLW-CI Phase 4: cognitive fusion weight
...
# ==========================================
# DLW-CI Phase 3: 动态小生境 (Niche) 构建函数
# ==========================================
...
# ==========================================
# DLW-CI Phase 4: 认知差异 (Cognitive Difference) 与信念融合函数
# ==========================================
...
# ==========================================
# DLW-CI Phase 5: 角色自适应 (Role-Adaptive) 函数
# ==========================================
...
# ==========================================
# DLW-CI Phase 6: 源声明与排斥 (Source Declaration & Exclusion) 函数
# ==========================================
...
ess_h = [[] for _ in range(layer_num)]  # DLW-CI Phase 1: 每层每步的有效粒子数 (ESS)
niche_h = []  # DLW-CI Phase 3: 每个时间步的小生境分组列表
fusion_weight_h = [[] for _ in range(layer_num)]  # DLW-CI Phase 4: 每步的融合权重记录
role_h = []  # DLW-CI Phase 5: 每个时间步的角色分配记录
...
declared_sources = []  # DLW-CI Phase 6: 已声明源位置列表 [(x, y, Q), ...]
declared_source_h = []  # DLW-CI Phase 6: 每个时间步声明的源数量
candidate_sources = []  # DLW-CI Phase 6 修复: 候选源 [{pos, counter, supporters}, ...]
mse_h = []
...
# DLW-CI Phase 3: 初始小生境为各机器人独立 (第1步尚无KL信息)
niches = [[i] for i in range(layer_num) if robot_status[i] == 1]

role_names = ['Scout', 'Tracker', 'Verifier']  # DLW-CI Phase 5: 角色名称映射
...
log_f.write("DLW-CI Simulation Log\n")
log_f.write("=" * 60 + "\n")
...
for T in range(1, TT + 1):
    ...
    # 4.1 Niche 已由上一步的 form_dynamic_niches 动态生成 (DLW-CI Phase 3)
    ...
    # DLW-CI Phase 4: 认知差异融合权重 (修复: niche内 + same-source gate)
    all_fusion_weights = {}
    ...
    for niche in niches:
        ...
        # 2. ESS-based adaptive resampling (DLW-CI Phase 1)
    ...
    # DLW-CI Phase 3: 动态小生境更新 (受 USE_DYNAMIC_NICHE 开关控制)
    ...
    # DLW-CI Phase 5: 角色自适应分配 (受 USE_ROLE_ADAPTATION 开关控制)
    ...
    # DLW-CI Phase 6: 统一 candidate→declared pipeline (受 USE_SOURCE_DECLARATION 开关控制)
    ...
    for niche in niches:
        ...
        for ln in active_members:
            ...
            # DLW-CI Phase 5: 角色感知偏置 (受 USE_ROLE_ADAPTATION 开关控制)
            if USE_ROLE_ADAPTATION:
                beta_tracker = 0.3
                beta_verifier = 0.15
                for idx in range(num_candidates):
            ...
            # DLW-CI Phase 6: 已声明源排斥惩罚 (受 USE_SOURCE_DECLARATION 开关控制)
    ...
    if T % 5 == 0 or T == 1 or T == TT:
        ...
        ax_main.set_title(rf'DLW-CI Dynamic Niches: [{niche_str}] | $T = {T}/{TT}$',
...
# 5.7 绘制认知差异融合权重演化图 (DLW-CI Phase 4: Figure Fusion Weights)
if USE_COGNITIVE_FUSION and len(fusion_weight_h[0]) > 0:
    ...
    plt.title(r'Evolution of Cognitive-Difference Belief Fusion Weights (DLW-CI Phase 4)',
...
# ==========================================
# DLW-CI: 生成 debug_summary.txt (消融调试摘要)
# ==========================================
...
summary_lines.append("DLW-CI Debug Summary")
summary_lines.append("=" * 60)
